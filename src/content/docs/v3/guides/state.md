...
Enabling a state store is as simple as writing a [`BACKEND` block](/v3/backends/) in your DDL. The most basic option is a
local file:
...
When you first add a state store, or after [out-of-band changes](/v3/guides/drift.md), seed it from the live database using the
[`refresh`](/v3/cli/commands/refresh/) command:
...
[`show`](/v3/cli/commands/show/) prints what the store currently holds, without touching the database:
...
run can leave a stale lock; clear it with [`force-unlock`](/v3/cli/commands/force-unlock/) once you're certain nothing is
still running.

## Detecting divergence

To check whether the live database has diverged from the recorded state, see [Detecting drift](/v3/guides/drift/).

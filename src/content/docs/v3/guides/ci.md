...
* a strict [exit-code contract](/v3/cli/exit-codes/),
* opt-in "did anything change?" signals,
* [offline planning](/v3/guides/state/) that needs no database at plan time,
* and built-in linting, validation and drifting checks.

## Always pass `--auto-approve` for applies

Write actions like [`apply`](/v3/cli/commands/apply/) and [`destroy`](/v3/cli/commands/destroy/) will prompt for confirmation
...
See [Environment variables](/v3/cli/environment-variables/).
...
If a state store is configured, this will run **without a database connection**. See [Offline planning & state](/v3/guides/state/).
...
See [Detecting drift](/v3/guides/drift/).
...
See the [full exit-code contract](/v3/cli/exit-codes/).

...
* **A database.** This tool is for managing database schemas after all. See [Providers](/v3/providers/) for supported databases.
...
Swap `bash` for `zsh`, `fish`, or `pwsh`. See the [`completion` command](/v3/cli/commands/completion/)
for manual installation and the `--uninstall-autocomplete` flag.

## Next steps

With the tool installed, head to the [Quickstart](/v3/start/quickstart/) to scaffold a project and apply your first schema.

...
at it with [`--directory`](/v3/cli/#global-flags). There is nothing to configure: no format (it's always [NSchema DDL](/v3/ddl/grammar/)) and no
directory or glob.
...
Errors exit `1`; a clean validation exits `0`. Warnings never fail the command. See the full [exit-code contract](/v3/cli/exit-codes/).

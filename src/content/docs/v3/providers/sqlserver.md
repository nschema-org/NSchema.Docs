...
[Environment variables](/v3/cli/environment-variables/#separate-credentials).
...
When [embedding the engine](/v3/library/embedding/) instead of the CLI, register SQL Server with the `NSchema.SqlServer`
package:
...
In [DDL](/v3/ddl/types/) you write these as ordinary type names (`money`, `xml`, `rowversion`); unrecognized names pass through as
custom types.

...
in CI) or to point at a different database file per [environment](/v3/cli/configuration/#environments) without editing the checked-in `.sql`:
...
`main` always exists, so the provider never creates or drops it: a [`destroy`](/v3/cli/commands/destroy/) removes the
...
* **Triggers** carry an inline body, written as `CREATE TRIGGER … ON main.t AS $$ BEGIN … END $$`, (see the [DDL grammar](/v3/ddl/grammar/#triggers))
...
When [embedding the engine](/v3/library/embedding/) instead of the CLI, register SQLite with the `NSchema.SQLite` package:

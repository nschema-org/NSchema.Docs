...
The local-file store is built into the engine, so it needs no `PLUGIN` declaration.
...
using something like the [Amazon S3 backend](/state/s3/) instead.

## Locking

NSchema locks the state file during write operations so concurrent runs can't corrupt it. See [Locking](/state/#locking).

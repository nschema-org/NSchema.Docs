...
:::tip
This section describes the language shape of config blocks. For the attributes the CLI recognizes, see [Configuration blocks](/v4/cli/configuration/),
[Providers](/v4/providers/), and [Backends](/v4/backends/).
:::
...
* The **event** is when the script runs: `PRE DEPLOYMENT` / `POST DEPLOYMENT` are fixed bookends around the computed
  ...
  contains the matching structural change, splicing the SQL at the change (see [Data migrations](/v4/guides/data-migrations/)).
* The **run condition** is how often it runs when its event occurs: `ALWAYS` (the default — a bare `RUN ON …` means
  ...
  [Deployment scripts](/v4/guides/deployment-scripts/#run-conditions)). `UNLESS EXISTS` is reserved for a future release.
* A change-event target is a three-segment path — the third segment is a column name for the column events and a
  ...
  [guide](/v4/guides/data-migrations/#migrations-in-templates)).
...
normalize to the canonical name (see [type reference](/v4/ddl/types/)). The modifier order above is fixed.
...
exclusive, and which one a [provider](/v4/providers/) accepts depends on the specific database. PostgreSQL uses
...
* A schema template body accepts `CREATE` object statements, table `GRANT`s, and [`SCRIPT` statements](/v4/guides/data-migrations/#migrations-in-templates)
...
objects. See [Templates](/v4/guides/templates/) for the practical guide.
:::

...
Declare it with a `BACKEND file` [config block](/v4/cli/configuration/):
...
using something like the [Amazon S3 backend](/v4/backends/s3/) instead.

## Locking

NSchema locks the state file during write operations so concurrent runs can't corrupt it. See [Locking](/v4/backends/#locking).

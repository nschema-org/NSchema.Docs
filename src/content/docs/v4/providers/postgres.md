...
sidebar:
  order: 20
slug: v4/providers/postgres
---

Declare a postgres provider using a `PROVIDER postgres` [config block](/v4/cli/configuration/):
...
discrete overrides are layered on top. See [Environment variables](/v4/cli/environment-variables/#separate-credentials).

## Dialect

The SQL dialect is determined by the provider, so there is nothing to configure. NSchema's canonical [DDL types](/v4/ddl/types/) are
...
When [embedding the engine](/v4/library/embedding/) instead of the CLI, register Postgres with the `NSchema.Postgres` package:
...
In [DDL](/v4/ddl/types/) you write these as ordinary type names (`citext`, `jsonb`); unrecognized names pass through as custom types.

...
sidebar:
  order: 25
slug: v4/providers/sqlserver
---

Declare a sqlserver provider using a `PROVIDER sqlserver` [config block](/v4/cli/configuration/):
...
[Environment variables](/v4/cli/environment-variables/#separate-credentials).
...
When [embedding the engine](/v4/library/embedding/) instead of the CLI, register SQL Server with the `NSchema.SqlServer`
package:
...
In [DDL](/v4/ddl/types/) you write these as ordinary type names (`money`, `xml`, `rowversion`); unrecognized names pass through as
custom types.

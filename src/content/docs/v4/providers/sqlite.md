...
sidebar:
  order: 30
slug: v4/providers/sqlite
---

Declare a sqlite provider using a `PROVIDER sqlite` [config block](/v4/cli/configuration/):
...
in CI) or to point at a different database file per [environment](/v4/cli/configuration/#environments) without editing the checked-in `.sql`:
...
`main` always exists, so the provider never creates or drops it: a [`destroy`](/v4/cli/commands/destroy/) removes the
...
* **Triggers** carry an inline body, written as `CREATE TRIGGER … ON main.t AS $$ BEGIN … END $$`, (see the [DDL grammar](/v4/ddl/grammar/#triggers))
...
When [embedding the engine](/v4/library/embedding/) instead of the CLI, register SQLite with the `NSchema.Sqlite` package:

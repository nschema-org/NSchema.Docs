...
A provider is the live database NSchema reads from and writes to. It's declared in a `PROVIDER` [config block](/v4/cli/configuration/) and
...
The provider is configured separately from the optional [backend](/v4/backends/), which persists a snapshot of the schema
for offline planning. This section covers the live-database providers; see [Backends](/v4/backends/) for the state backends.
...
package with a `source` attribute (see [Configuration](/v4/cli/configuration/#plugins-and-versions)).

## Available providers

| Provider   | Page                                          |
|------------|-----------------------------------------------|
| Postgres   | [Postgres provider](/v4/providers/postgres/)     |
| SQL Server | [SQL Server provider](/v4/providers/sqlserver/)  |
| SQLite     | [SQLite provider](/v4/providers/sqlite/)         |

## How a provider relates to the DDL

The [DDL you write](/v4/ddl/defining-schemas/) is dialect-agnostic by design: canonical type names like `bigint` and

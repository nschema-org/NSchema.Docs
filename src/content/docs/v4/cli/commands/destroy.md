...
`destroy` is purely destructive and is **exempt** from the[destructive-action policy](/v4/guides/destructive-actions/). To preview exactly what it
would drop without running it, use [`plan --destroy`](/v4/cli/commands/plan/#previewing-a-teardown).
:::
...
Like a declined [`apply`](/v4/cli/commands/apply/), a declined `destroy` makes no changes and exits `1`.

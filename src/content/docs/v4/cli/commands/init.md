...
sidebar:
  order: 2
slug: v4/cli/commands/init
---

Restore the provider and backend [plugins](/v4/cli/configuration/#plugins-and-versions) pinned by your project, so a later command doesn't have to.
...
To create a *new* project's files, use [`scaffold`](/v4/cli/commands/scaffold/) instead — `init` only restores plugins for an existing project.

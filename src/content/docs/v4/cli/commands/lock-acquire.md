...
sidebar:
  order: 13.2
slug: v4/cli/commands/lock-acquire
---
...
can write while you prepare. Release it with [`lock release`](/v4/cli/commands/lock-release/) when you're done.
...
* **`--reason <text>`** — annotate the lock with a human-readable reason, surfaced by [`lock status`](/v4/cli/commands/lock-status/)
  so others know why the state is held.
* **`--ttl <duration>`** — record an optional expiry on the lock (for example `90s`, `30m`, `2h`, `1d`). [`lock status`](/v4/cli/commands/lock-status/)

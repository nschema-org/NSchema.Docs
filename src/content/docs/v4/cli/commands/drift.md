...
* **`--detailed-exitcode`** — return a [detailed exit code](/v4/cli/exit-codes/): `0` when there is no drift, `2` when the live database
* has drifted (errors stay `1`), so a monitoring job can gate on it. Without it, `drift` exits `0` and you read the diff it prints.

See [Detecting drift](/v4/guides/drift/) for how to use this in monitoring.

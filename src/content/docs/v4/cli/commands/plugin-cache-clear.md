...
sidebar:
  order: 14.33
slug: v4/cli/commands/plugin-cache-clear
---

Remove **everything** from the shared plugin cache (`~/.nschema/plugins`) in one step. For removing a single package
instead, see [`plugin cache remove`](/v4/cli/commands/plugin-cache-remove/).
...
front with [`init`](/v4/cli/commands/init/). The destructive "remove everything" lives in this dedicated command rather than

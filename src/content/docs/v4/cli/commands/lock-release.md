---
title: lock release
description: Release a held state-store lock, including a stale one left by an
  interrupted run.
sidebar:
  order: 13.3
slug: v4/cli/commands/lock-release
---
...
The `lock-id` is shown by [`lock status`](/v4/cli/commands/lock-status/), by [`lock acquire`](/v4/cli/commands/lock-acquire/),
...
* **`lock-id`** — the id of the lock to release, copied from [`lock status`](/v4/cli/commands/lock-status/),
  [`lock acquire`](/v4/cli/commands/lock-acquire/), or the blocked operation's error. The release is refused if it no longer

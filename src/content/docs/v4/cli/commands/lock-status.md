...
sidebar:
  order: 13.1
slug: v4/cli/commands/lock-status
---

Show whether the [state store](/v4/guides/state/) is currently locked. It reads the lock, if there is one, and surfaces who
...
If the lock records a [`--ttl`](/v4/cli/commands/lock-acquire/), the expiry is surfaced here too (it is informational only:
NSchema never auto-enforces it). For a broader health check (database + state store + lock together) see [`doctor`](/v4/cli/commands/doctor/).

## Options

* **`--detailed-exitcode`** — return a [detailed exit code](/v4/cli/exit-codes/): `0` when the state is not locked,

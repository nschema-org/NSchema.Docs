...
description: Remove a plugin package from the shared cache, by package id and
  optional version.
sidebar:
  order: 14.32
slug: v4/cli/commands/plugin-cache-remove
---
...
Use the package ids and versions shown by [`plugin cache list`](/v4/cli/commands/plugin-cache-list/). If nothing matches,
the command says so and changes nothing. Removal is **not** prompted: the cache is a restorable copy, so anything removed
is re-fetched on its next use or with [`init`](/v4/cli/commands/init/).

To wipe the cache entirely, use [`plugin cache clear`](/v4/cli/commands/plugin-cache-clear/) instead of naming every package.

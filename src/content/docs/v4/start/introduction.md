...
description: NSchema is a declarative database schema migration tool. Describe
  the schema you want; NSchema computes and applies the migration to get there.
sidebar:
  order: 1
slug: v4/start/introduction
---
...
One key difference though, is that databases are inherently *stateful*. Most accidentally destroyed infrastructure
...
and the exact SQL that will be executed first. You can even [save a plan to a file](/v4/cli/commands/plan/) and apply
...
domains and composite types. For anything more niche, NSchema also supports arbitrary [pre-deployment and post-deployment scripts](/v4/guides/deployment-scripts).

It's important to note that while your NSchema scripts might *look* like SQL, it's actually a provider-neutral DSL meant
...
is what most people want, and what the bulk of these docs cover. Start with [Installation](/v4/start/installation/) and the
[Quickstart Guide](/v4/start/quickstart/).
...
See [Embedding the engine](/v4/library/embedding/).

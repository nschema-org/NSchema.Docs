...
* a strict [exit-code contract](/v4/cli/exit-codes/),
* opt-in "did anything change?" signals,
* [offline planning](/v4/guides/state/) that needs no database at plan time,
* and built-in linting, validation and drifting checks.

## Always pass `--auto-approve` for applies

Write actions like [`apply`](/v4/cli/commands/apply/) and [`destroy`](/v4/cli/commands/destroy/) will prompt for confirmation
...
While the `PROVIDER` block supports setting the connection string and password directly, it is *strongly* recommended to
...
See [Environment variables](/v4/cli/environment-variables/).

## Plugins are restored on first use

Providers and backends are [plugins](/v4/cli/configuration/#plugins-and-versions): `nschema` downloads the version pinned in
...
If a state store is configured, this will run **without a database connection**. See [Offline planning & state](/v4/guides/state/).
...
See [Detecting drift](/v4/guides/drift/).
...
See the [full exit-code contract](/v4/cli/exit-codes/).

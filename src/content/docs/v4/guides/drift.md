...
sidebar:
  order: 40
slug: v4/guides/drift
---

Drift is when the live database no longer matches NSchema's latest snapshot in the state store. This happens when people
make manual schema changes outside of NSchema, and it can cause data loss if not handled carefully. The [`nschema drift`](/v4/cli/commands/drift/)
...
:::note[Needs]
A live database **and** a [state store](/v4/guides/state/) to compare against. If you don't have one configured, see [Offline planning & state](/v4/guides/state/).
:::
...
Once you've reviewed the drift, you can choose to either adopt the drift using [`refresh`](/v4/cli/commands/refresh/), then
updating your `.sql` files to match, or you can revert the drift using [`apply`](/v4/cli/commands/apply/) to bring the

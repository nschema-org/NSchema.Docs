---
title: Data hazards
description: How NSchema warns about changes that are valid against the schema
  but can fail on the data already in a table.
sidebar:
  order: 65
slug: v4/guides/data-hazards
---
...
The option applies to both [`plan`](/v4/cli/commands/plan/) and [`apply`](/v4/cli/commands/apply/).

## Resolving a hazard with a migration script

Declaring a matching [change-event `SCRIPT`](/v4/guides/data-migrations/) silences the corresponding hazard: the script states
...
Like every diff policy, data-hazard detection applies to forward migrations. A teardown ([`destroy`](/v4/cli/commands/destroy/),
[`plan --destroy`](/v4/cli/commands/plan/)) bypasses the diff and its policies entirely.

...
sidebar:
  order: 50
slug: v4/guides/deployment-scripts
---
...
(e.g. `CREATE INDEX CONCURRENTLY`). See the [grammar reference](/v4/ddl/grammar/#scripts) for the full syntax.
...
[state backend](/v4/guides/state/), so later plans can skip it:
...
* **Editing an executed script doesn't re-run it.** The plan warns that the body has changed, but still skips it. To run
  the new body once, [`script taint`](/v4/cli/commands/script-taint/) it, or rename it (a new name is a new identity).
...
The scripts are managed with the [`script` command group](/v4/cli/commands/script/):
[`script list`](/v4/cli/commands/script-list/) shows what has run, [`script taint`](/v4/cli/commands/script-taint/) forgets
an execution so the script runs again, and [`script untaint`](/v4/cli/commands/script-untaint/) records a pending script
as executed without running it (e.g. after [rebuilding lost state](/v4/guides/state/#state-surgery)).

## Scripts in templates

A [schema template](/v4/guides/templates/) can declare deployment scripts, instantiated once per applied schema, with the
...
[change-event script](/v4/guides/data-migrations/) (`SCRIPT … RUN ON ADD COLUMN …`): it runs only when its matching
